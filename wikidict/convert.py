...
class BaseFormat:
    ...
    def get_lang_destination(self) -> str:
        return self.lang_dst

    def get_lang_source(self) -> str:
...
<xdxf revision="034">
    <meta_info>
        <languages>
            <from xml:lang="{self.get_lang_source()}"/>
            <to xml:lang="{self.get_lang_destination()}"/>
        </languages>
...
class ConverterFromDictFile(DictFileFormat):
    ...
    def _convert(self) -> None:
        ...
        glos.sourceLangName = self.get_lang_source()
        glos.targetLangName = self.get_lang_destination()
...
class PocketBookFormat(ConverterFromDictFile):
    """Save the data into a PocketBook file."""

    target_format = "pocketbook"
    target_suffix = "dic"
...
def get_secondary_formatters() -> list[type[BaseFormat]]:
    ...
    return [BZ2DictFileFormat, DictOrgFormat, PocketBookFormat, StarDictFormat]
